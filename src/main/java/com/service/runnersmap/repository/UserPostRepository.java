...
public interface UserPostRepository extends JpaRepository<UserPost, UserPostPK> {
  ...
  @Query("SELECT up FROM UserPost up WHERE up.id.postId.postId = :postId AND up.valid_yn = true")
  List<UserPost> findByPostId(@Param("postId") Long postId);

  @Query("SELECT SUM(up.totalDistance) FROM UserPost up WHERE up.id.userId.id = :userId AND up.valid_yn = true")
  Double findTotalDistanceByUserId(@Param("userId") Long userId);

  @Query("SELECT SUM(up.totalDistance) FROM UserPost up WHERE up.id.userId.id = :userId AND up.valid_yn = true AND FUNCTION('YEAR', up.actualEndTime) = :year AND FUNCTION('MONTH', up.actualEndTime) = :month")
  Double findTotalDistanceByUserIdAndMonth(@Param("userId") Long userId, @Param("year") int year, @Param("month") int month);

  @Query("SELECT up " +
         "FROM  UserPost up " +
         "WHERE up.id.userId.id = :userId "+
